			...
			<div id="no_wInfo" class="panel-collapse collapse in">
				<div class="panel-body">
					<table class="table table-hover" style="margin-bottom: 0px">
						...
						<tbody>
							<#assign i=0>
							<#if wSMPageBean?exists>
							<#list wSMPageBean.list as message>
							<#assign i=i+1>
							<tr>
								<td>${message.rName}</td>
								<td>${message.sName}</td>
								...
								<td>${message.mark}</td>
								<td>
									<a href="#" data-toggle="modal" data-target="#message${i}"
										style="padding-left: 8px;"> 
										<i class="glyphicon glyphicon-list-alt"></i>
									</a>
								</td>
								<td>
									<a href="javascript:showAtRight('${base}/message/answerM?state=1&uId=${user.id}&mId=${message.addressee}');"
										style="padding-left: 8px;"> 
										<i class="glyphicon glyphicon-edit"></i>
									</a>
								</td>
								<td>
									<a href="javascript:showAtRight('${base}/message/deleteSM?num=${i}&currPage=${wSMPageBean.currPage}&mId=${message.id}&uId=${user.id}');"
										style="padding-left: 8px;"> 
					...
					<#if wSMPageBean.totalPage != 0>
						<#if wSMPageBean.totalPage <= 2>
							<ul class="pagination" style="padding: 0px;margin:0;">
								<#if wSMPageBean.currPage == 1>
									<li>
										<a href="#" class="active">首页</a>
									</li>
								<#else>
								<li>
										<a href="javascript:showAtRight('${base}/message/querySM?id=${user.id}&currPage=1');">首页</a>
									</li>
								</#if>
								
								<#list 1..wSMPageBean.totalPage as n>
									<li><a href="javascript:showAtRight('${base}/message/querySM?id=${user.id}&currPage=${n}');">${n}</a></li>
								</#list>
								<li>
									<a href="javascript:showAtRight('${base}/message/querySM?id=${user.id}&currPage=${wSMPageBean.totalPage}');">尾页</a>
								</li>
							</ul>
						</#if>
						<#if wSMPageBean.totalPage gt 2>
							<#assign i=1>
							<ul class="pagination" style="padding: 0px;margin:0;">
								<#if wSMPageBean.currPage gt 1>
								<li><a href="javascript:showAtRight('${base}/message/querySM?id=${user.id}&currPage=${wSMPageBean.currPage-i}');">上一页</a></li>
								<#else>
								<li><a href="#">上一页</a></li>
								</#if> 
								<li><a href="#">${wSMPageBean.currPage}</a></li>
								<#if wSMPageBean.currPage == wSMPageBean.totalPage>
								<li><a href="#">下一页</a></li>
								<#else>
								<li><a href="javascript:showAtRight('${base}/message/querySM?id=${user.id}&currPage=${wSMPageBean.currPage+i}');">下一页</a></li>
								</#if>
							</ul>
						</#if>
					<ul class="pagination" style="margin:0 0 0 20px;padding: 0px;">
					<li><a href="#" class="active">第</a></li>
					<li><a href="#">${wSMPageBean.currPage} / ${wSMPageBean.totalPage}</a></li>
