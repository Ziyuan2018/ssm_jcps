			...
			<div id="no_wInfo" class="panel-collapse collapse in">
				<div class="panel-body">
					<table class="table table-hover" style="margin-bottom: 0px">
						...
						<tbody>
							<#assign i=0>
							<#if SMPageBean?exists>
							<#list SMPageBean.list as message>
							<#assign i=i+1>
							<tr>
								<td>${message.rName}</td>
								...
								<td>
									<a href="javascript:showAtRight('${base}/message/deleteSM?num=${i}&currPage=${SMPageBean.currPage}&mId=${message.id}&uId=${user.id}');"
										style="padding-left: 8px;"> 
					...
					<#if SMPageBean.totalPage != 0>
						<#if SMPageBean.totalPage <= 2>
							<ul class="pagination" style="padding: 0px;margin:0;">
								<#if SMPageBean.currPage == 1>
									<li>
										<a href="#" class="active">首页</a>
									</li>
								<#else>
								<li>
										<a href="javascript:showAtRight('${base}/message/querySM?uId=${user.id}&currPage=1');">首页</a>
									</li>
								</#if>
								
								<#list 1..SMPageBean.totalPage as n>
									<li><a href="javascript:showAtRight('${base}/message/querySM?uId=${user.id}&currPage=${n}');">${n}</a></li>
								</#list>
								<li>
									<a href="javascript:showAtRight('${base}/message/querySM?uId=${user.id}&currPage=${SMPageBean.totalPage}');">尾页</a>
								</li>
							</ul>
						</#if>
						<#if SMPageBean.totalPage gt 2>
							<#assign i=1>
							<ul class="pagination" style="padding: 0px;margin:0;">
								<#if SMPageBean.currPage gt 1>
								<li><a href="javascript:showAtRight('${base}/message/querySM?uId=${user.id}&currPage=${SMPageBean.currPage-i}');">上一页</a></li>
								<#else>
								<li><a href="#">上一页</a></li>
								</#if> 
								<li><a href="#">${SMPageBean.currPage}</a></li>
								<#if SMPageBean.currPage == SMPageBean.totalPage>
								<li><a href="#">下一页</a></li>
								<#else>
								<li><a href="javascript:showAtRight('${base}/message/querySM?uId=${user.id}&currPage=${SMPageBean.currPage+i}');">下一页</a></li>
								</#if>
							</ul>
						</#if>
					<ul class="pagination" style="margin:0 0 0 20px;padding: 0px;">
					<li><a href="#" class="active">第</a></li>
					<li><a href="#">${SMPageBean.currPage} / ${SMPageBean.totalPage}</a></li>
