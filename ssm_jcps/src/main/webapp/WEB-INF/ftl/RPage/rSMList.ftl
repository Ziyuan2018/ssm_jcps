			...
			<div id="no_wInfo" class="panel-collapse collapse in">
				<div class="panel-body">
					<table class="table table-hover" style="margin-bottom: 0px">
						...
						<tbody>
							<#assign i=0>
							<#if rSMPageBean?exists>
							<#list rSMPageBean.list as message>
							<#assign i=i+1>
							<tr>
								<td>${message.rName}</td>
								...
								<td>
									<a href="javascript:showAtRight('${base}/message/deleteSM?num=${i}&currPage=${rSMPageBean.currPage}&mId=${message.id}&uId=${user.id}');"
										style="padding-left: 8px;"> 
					...
					<#if rSMPageBean.totalPage != 0>
						<#if rSMPageBean.totalPage <= 2>
							<ul class="pagination" style="padding: 0px;margin:0;">
								<#if rSMPageBean.currPage == 1>
									<li>
								...
								<#list 1..rSMPageBean.totalPage as n>
									<li><a href="javascript:showAtRight('${base}/message/querySM?id=${user.id}&currPage=${n}');">${n}</a></li>
								</#list>
								<li>
									<a href="javascript:showAtRight('${base}/message/querySM?id=${user.id}&currPage=${rSMPageBean.totalPage}');">尾页</a>
								</li>
							</ul>
						</#if>
						<#if rSMPageBean.totalPage gt 2>
							<#assign i=1>
							<ul class="pagination" style="padding: 0px;margin:0;">
								<#if rSMPageBean.currPage gt 1>
								<li><a href="javascript:showAtRight('${base}/message/querySM?id=${user.id}&currPage=${rSMPageBean.currPage-i}');">上一页</a></li>
								<#else>
								<li><a href="#">上一页</a></li>
								</#if> 
								<li><a href="#">${rSMPageBean.currPage}</a></li>
								<#if rSMPageBean.currPage == rSMPageBean.totalPage>
								<li><a href="#">下一页</a></li>
								<#else>
								<li><a href="javascript:showAtRight('${base}/message/querySM?id=${user.id}&currPage=${rSMPageBean.currPage+i}');">下一页</a></li>
								</#if>
							</ul>
						</#if>
					<ul class="pagination" style="margin:0 0 0 20px;padding: 0px;">
					<li><a href="#" class="active">第</a></li>
					<li><a href="#">${rSMPageBean.currPage} / ${rSMPageBean.totalPage}</a></li>
